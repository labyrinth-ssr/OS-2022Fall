...
void init_slab_node(SlabNode* slab_node,int obj_num,bool first,CacheNode* owner_cache){
    ...
    // (void*)((u64)slab_node+sizeof(SlabNode)+(sizeof(int)*obj_num/8+1)*8) : owner_cache;
...
// printk("cpu %d , colour: %d",cpuid(),slab_node->colour);
    ...
    // }
    return (void*)((u64)slab_node+sizeof(SlabNode)+ (sizeof(int)*obj_num/8+1)*8+slab_node->colour+(slab_node->freelist[slab_node->active++])*size );
}
...
void add_to_cache_queue(CacheNode* head,isize size,CacheNode* cache_node,bool first_node,int num,SlabNode* slabs_partial){
    ...
    // (SlabNode*)((u64)cache_node-sizeof(SlabNode)-(sizeof(int)*(cache_node->num)/8+1)*8);
    ...
    if (first_node)
    {
        init_list_node(cache_node->self);
    } else
    {
...
void* kalloc(isize size)
{
    ...
    if (NULL==kmem_cache_head||NULL== search_res)
    {
        ...
        // (PAGE_SIZE-3*COLOUR_OFF-sizeof(SlabNode))/(size+sizeof(int));
        ...
        // (CacheNode*)((u64)free_head+sizeof(SlabNode)+(sizeof(int)*num/8+1)*8);
        if (NULL==kmem_cache_head)
        {
            kmem_cache_head=offset_node;
            // add_to_cache_queue(kmem_cache_head,size,(CacheNode*)offset_node,true);
        ...
        // {
            // printk("add cacahe:");
            add_to_cache_queue(kmem_cache_head,size,(CacheNode*)offset_node,NULL==kmem_cache_head,num,free_head);
        // }
        // printk("cpu %d , free_head %p ,num %lld\n",cpuid(),free_head,num);
